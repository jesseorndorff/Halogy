...
class Events extends MX_Controller {
	...
	function __construct()
	{
		...
		if ($data['month'] = $this->events->get_month($month, $year))
		{
			...
			{
				$monthEvents[date('j', strtotime($event['eventDate']))] = '/events/'.date('Y/m/d', strtotime($event['eventDate']));
			}
		}
	...
	function viewevent($eventID = '')
	{
		...
		{				
			...
			$output['event:edit'] = ($event['userID'] == $this->session->userdata('userID')) ? ' | '.anchor('/ce/events/edit_event/'.$eventID, 'Edit Event') : '';
