			...
			<?php if ($versions): ?>
				<?php foreach($versions as $version): ?>
					<li>
						...
						<?php else: ?>
							<?php echo dateFmt($version['dateCreated'], '', '', TRUE).(($user = $this->core->lookup_user($version['userID'], TRUE)) ? ' <em>(by '.$user.')</em>' : ''); ?> <?php echo anchor('/admin/pages/revert_template/'.$version['objectID'].'/'.$version['versionID'], 'Revert', array('class' => 'button small grey right', 'onClick' => 'return confirm(\'You will lose unsaved changes. Continue?\');')); ?>
						<?php endif; ?>
					</li>
				<?php endforeach; ?>
			<?php endif; ?>
