		...
		<div class="header">
			...
			<div class="large-6 small-12 columns right">
				<?php if ($this->site->config['plan'] = 0 || $this->site->config['plan'] = 6 || (($this->site->config['plan'] > 0 && $this->site->config['plan'] < 6) && $quota < $this->site->plans['storage'])): ?>
					<a href="#" class="button small radius green toggle" data-reveal-id="upload-file">Upload File</a>
				<?php endif; ?>
			</div>
		</div>
		<div class="row table">
...
<div id="upload-file" class="hidden clear reveal-modal" data-reveal>
	<form method="post" action="<?php echo site_url($this->uri->uri_string()); ?>" enctype="multipart/form-data" class="custom">
		...
		<input type="submit" value="Upload File" class="button small radius nolabel" id="submit" />
		<a href="<?php echo site_url('/admin/files'); ?>" class="button small radius cancel grey">Cancel</a>
